...
void RubbishHtmlParser::parse(const char *html, int length)
{
  ...
  // Bound recursion depth. main_task runs on a 32 KB stack and each frame here
  // carries a std::function plus pugi node copies; pathologically nested markup
  // (deeply wrapped <div>/<span> chains) could otherwise overflow the stack and
  // panic the device with no log. Past the cap we stop descending but still
  // capture text via enter/exit on the way back, so content isn't lost -- only
  // the (cosmetic) styling of absurdly deep nodes is.
  static const int MAX_WALK_DEPTH = 64;
