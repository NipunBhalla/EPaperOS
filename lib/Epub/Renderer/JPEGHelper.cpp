...
bool JPEGHelper::render(const uint8_t *data, size_t data_size, Renderer *renderer, int x_pos, int y_pos, int width, int height)
{
  ...
  if (res == JDR_OK)
  {
    this->x_scale = float(width) / float(dec.width);
    this->y_scale = float(height) / float(dec.height);
...
)
{
  ...
  for (int y = rect->top; y <= rect->bottom; y++)
  {
    for (int x = rect->left; x <= rect->right; x++)
    {
      ...
      // draw_image_pixel applies contrast-stretch + dither when enhance is on.
      renderer->draw_image_pixel(context->x_pos + x * context->x_scale,
                                 context->y_pos + y * context->y_scale, gray);
    }
  }
  return 1;
}
